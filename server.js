...
function requireMonimeConfig() {
  if (!isNonEmptyString(process.env.MONIME_API_TOKEN)) {
    throw new AppError(
      "MONIME_API_TOKEN is not configured on the server.",
      500
    );
  }
...
async function monimeRequest(path, options = {}) {
  ...
  const headers = {
    ...
    Authorization: `Bearer ${process.env.MONIME_API_TOKEN}`,
...
app.get("/health", (_req, res) => {
  const token = process.env.MONIME_API_TOKEN || "";
  res.json({
    ok: true,
    service: "monime-mobile-money-deposits",
    mobileMoneyOnly: true,
    monime: {
      ...
      environment: token.startsWith("mon_test_")
        ? "test"
        : token.startsWith("mon_")
          ? "live"
          : "unknown",
    },
